...
function createAdapterUtils(orm) {
  ...
  function getPropertyMetadata(metadata2, fieldName) {
    const prop = metadata2.props.find(prop2 => {
      ...
      if (
        prop2.kind === import_core.ReferenceKind.MANY_TO_ONE &&
  ...
  const getFieldPath = (metadata2, fieldName, throwOnShadowProps = false) => {
    ...
    if (
      prop.kind === import_core.ReferenceKind.SCALAR ||
      prop.kind === import_core.ReferenceKind.EMBEDDED
    ) {
      return [prop.name]
    }
    if (prop.kind === import_core.ReferenceKind.MANY_TO_ONE) {
  ...
  const normalizePropertyValue = (property, value) => {
    if (
      !property.targetMeta ||
      property.kind === import_core.ReferenceKind.SCALAR ||
      property.kind === import_core.ReferenceKind.EMBEDDED
    ) {
      return value
    }
  ...
  function createWhereClause(path, value, op, target = {}) {
    ;(0, import_dset.dset)(
      target,
      op == null || op === "eq" ? path : path.concat(op),
      value
    )
    return target
  }
...
var mikroOrmAdapter = (orm, {debugLogs, supportsJSON = true} = {}) =>
  (0, import_adapters.createAdapter)({
    config: {
      debugLogs,
      supportsJSON,
